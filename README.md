...
Developed a Python-based Insider Threat Detection Tool to monitor file access, off-hours system activity, USB insertions, and suspicious processes. Implemented real-time event logging using SQLite and built a Tkinter GUI to support host-based security monitoring, log analysis, and incident investigation.
...
Off-Hours Login: Detects activity outside of 9 AM – 6 PM.
Mass File Access: Alerts if >3 sensitive files are accessed within a minute.
Suspicious Processes: Alerts if >5 dangerous processes (e.g., cmd.exe, powershell.exe) are running.
Physical Security: Immediate alert upon USB device insertion.
...
Install Dependencies You must install customtkinter and psutil to run:
...
python INTD.py
⚙️ Configuration
Ensure Python 3 is installed. Tested on Windows 11. Modify paths for compatibility with other operating systems. Run with administrator privileges for full functionality. You can customize the monitoring rules by editing the CONFIG section in INTD.py:
